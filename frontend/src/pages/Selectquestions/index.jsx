...
import { selectSpheres, selectQuestions } from '../../store/store';
import { createQuestionsThunk } from '../../features/questions/questionSlice';
...
export default function SphereQuestionsAligned() {
    ...
    const handleSave = async () => {
        if (!spheres.current) {
            // sphere should exist from create flow
            return;
        }
        await dispatch(
            createQuestionsThunk({
                sphereId: spheres.current.id || spheres.current._id,
                questions,
            }),
        );
    };
        ...
        <div className="h-screen w-full bg-[#FFFDF0] font-sans text-stone-900 flex overflow-hidden">
            ...
            <aside className="w-[350px] flex flex-col border-r-4 border-stone-200 border-dashed bg-white z-20 shadow-xl flex-shrink-0">
                ...
                <div className="h-20 px-4 border-t-4 border-stone-200 border-dashed bg-[#FFFDF0] flex items-center justify-center flex-shrink-0">
                    <button
                        onClick={handleSave}
                        className="w-full bg-stone-900 text-yellow-400 font-black text-xs py-3 rounded-xl shadow-[3px_3px_0px_0px_#facc15] hover:translate-y-0.5 hover:shadow-none active:translate-y-1 transition-all flex justify-center items-center gap-2 border-2 border-stone-900"
                    >
                        <Save size={16} strokeWidth={3} /> {questionsState.status === 'loading' ? 'SAVING...' : 'SAVE PROGRESS'}
                    </button>
                </div>
            </aside>
